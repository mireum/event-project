...
function ReservInfo(props) {
  ...
  useEffect(() => {
    const reserv = async () => {
      try {
        const result = await axios.get(`${process.env.REACT_APP_SERVER_ADDR}/user/reserv/info`, {params: {userId}})
        console.log(result);
  ...
  const handleSubmitCancelPay = async () => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_SERVER_ADDR}/post/reserv/delete`, {cancelId});
      const result = await axios.get(`${process.env.REACT_APP_SERVER_ADDR}/user/reserv/info`, {params: {userId}})
      setReserv(result.data)
    } catch (err) {
      console.error(err);
